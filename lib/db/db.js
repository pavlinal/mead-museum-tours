  ...
  pool.getConnection(function(err, connection){
    if (err) {
      ...
      if (field === 'Object name'){
        sql = 'select * from objects where title = " '+query+'"';
      }
      else if (field === 'Artist name'){
        sql = 'select * from objects where makers = "'+query+'"';
      }
      else if (field === 'Tour name'){
        sql = 'select * from objects where tour = "'+query+'"';
      }
      else if (field === 'Culture'){
        sql = 'select * from objects where culture = "'+query+'"';
      }
      else if (field === 'Object ID'){
        sql = 'select * from objects where id = " '+query+' "';
      }
      var values = [query+""];
